...
const ShareModal = ({ file, onClose }) => {
    ...
    const handleShare = async (e) => {
        ...
        try {
            await sharedService.shareFile(file.id, email.trim(), role);
            toast.success(`"${file.name}" shared with ${email}`, { id: toastId });
            onClose();
        } catch (error) {
            toast.error(error?.response?.data?.message || "Share failed", { id: toastId });
        } finally {
            setLoading(false);
        }
    };
                ...
                <form onSubmit={handleShare} className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-[5px]">
                        ...
                        <div className="flex items-center gap-2 p-1
                            bg-slate-100/80 rounded-[10px]">
                            {["viewer", "editor"].map((r) => (
                                <button
                                    key={r}
                                    type="button"
                                    onClick={() => setRole(r)}
                                    className={`flex-1 py-1.5 rounded-[8px] cursor-pointer
